...
export default function SavedScreen() {
  ...
  /* перерисовывать экран при возврате */
  useFocusEffect(React.useCallback(() => {}, [saved]));

  const EmptyState = () => (
    <View style={styles.emptyWrap}>
      <Text style={styles.emptyTxt}>You haven’t saved any{'\n'}places yet.</Text>
    </View>
  );
    ...
    <View style={styles.container}>
      ...
      {saved.length === 0 ? (
        ...
        <ScrollView contentContainerStyle={styles.body}>
          {saved.map((spot) => {
            /* ⇢ как в LocationDetails: переходим на MapScreen и открываем нужный спот */
            const onMap = () =>
              ...
              <View key={spot.name} style={styles.card}>
                ...
                <View style={styles.cardBody}>
                  ...
                  <View style={styles.inlineBtns}>
                    {/* Open on map ― как на других экранах */}
                    <TouchableOpacity style={styles.mapBtn} onPress={onMap} activeOpacity={0.85}>
                    ...
                    {/* Share */}
                    <TouchableOpacity style={styles.squareBtn} onPress={onShare} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
  ...
  /* header (идентично другим экранам) */
  header: {
    height: HEADER_H,
  ...
  h1:       { flex: 1, textAlign: 'center', color: '#FFFFFF', fontSize: 28, fontWeight: '700', marginBottom: -40 },
  ...
  saveBtn:         { marginRight: 12 },           // отступ до Share
  squareBtnActive: { backgroundColor: '#20769C' },// «saved»
